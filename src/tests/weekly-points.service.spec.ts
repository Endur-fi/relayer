...
describe("WeeklyPointsService", () => {
  ...
  describe("processWeeklyPointsForAllUsers", () => {
    it("should process points for all users", async () => {
      ...
      jest
        .spyOn(service as any, "calculateWeeklyPointsDelta")
        .mockResolvedValue(mockWeeklyDelta);
  ...
  describe("calculateWeeklyPointsDelta", () => {
    it("should correctly calculate weekly points delta", async () => {
      ...
      prisma.cumulative_weekly_snapshot_points.findFirst.mockResolvedValue(
        mockSnapshot
      );
      prisma.cumulative_weekly_snapshot_points.findMany.mockResolvedValue(
        mockPreviousSnapshots
      );
      ...
      console.log(
        "findMany calls:",
        prisma.cumulative_weekly_snapshot_points.findMany.mock.calls
      );
      console.log(
        "currentPoints calls:",
        prisma.points_aggregated.findMany.mock.calls
      );
      ...
      expect(
        prisma.cumulative_weekly_snapshot_points.findFirst
      ).toHaveBeenCalledWith({
        orderBy: { week_start_date: "desc" },
      });
      expect(
        prisma.cumulative_weekly_snapshot_points.findMany
      ).toHaveBeenCalledWith({
  ...
  describe("processUserPoints", () => {
    it("should successfully process user points", async () => {
      ...
      await (service as any).processUserPoints(
        mockUser,
        "0x1",
        { "0x1": "15.00" },
        mockResults
    ...
    it("should handle failed points processing", async () => {
      ...
      botService.sendWeeklyPointsEvent.mockRejectedValue(
