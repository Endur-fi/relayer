...
export class EkuboResolver {
  ...
  async getEkuboPositionsByUser(
    ...
    @Arg('fromDateTime', () => Date, { defaultValue: new Date('2024-11-24') }) fromDateTime: Date = new Date('2024-11-24'),
    ...
    if (showClosed) {
      return this.getAllUserPositionsWithinTimeframe(_userAddress, fromTimestamp, toTimestamp);
    } else {
      return this.getCurrentlyOwnedPositions(_userAddress, fromTimestamp, toTimestamp);
    }
  }
  ...
  private async getAllUserPositionsWithinTimeframe(
    ...
    const positions = await this.getPositionsInTimeframe(nftIds, fromTimestamp, toTimestamp);
    
    return this.enrichPositionsWithOwnership(positions, userAddress, toTimestamp);
  }
  ...
  private async getCurrentlyOwnedPositions(
    ...
    const ownedNftIds = await this.getCurrentlyOwnedNftIds(userAddress, toTimestamp);

    if (ownedNftIds.length === 0) {
      return [];
    }

    const positions = await this.getPositionsInTimeframe(ownedNftIds, fromTimestamp, toTimestamp);
  ...
  private async getUserNftIds(userAddress: string, toTimestamp: number): Promise<string[]> {
  ...
  private async getCurrentlyOwnedNftIds(userAddress: string, toTimestamp: number): Promise<string[]> {
  ...
  private groupPositionsByNftId(positions: PrismaPosition[]): Map<string, PrismaPosition[]> {
  ...
  private findRelevantPositionForTimeframe(
    ...
    for (let i = 0; i < positions.length; i++) {
      ...
      if (currentPosition.timestamp >= fromTimestamp && currentPosition.timestamp <= toTimestamp) {
