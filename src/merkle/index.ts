...
import { standariseAddress } from '../common/indexerUtils';
import { MerkleTree as MerkleTreeSNF } from './lib';

const values = [
  [
    '0x07a2619df13384228adb42e525a75ee853a0d2aeffcb89dd4257d5112e0a90c2', // user address
    '9000000000000000000', // user allocation
  ],
];
...
function getProof(leafIndex: number): string[] {
  return tree4.address_calldata(BigInt(num.getDecimalString(values[leafIndex][0].toString())))
    .proof;
}

async function checkProofValidity(user: string, amount: string, proof: string[], myRoot: string) {
  const addr = '0x232a79c2b164fd81c8da6b52c536d5004d6428e1fbbee44f43f784ca0ea64ff';
  ...
  const computedRoot: any = await contract.call('get_root_for', [user, amount, proof]);
  console.log('Computed Root:', num.getHexString(computedRoot));
  assert(num.getHexString(computedRoot) === myRoot, 'Computed root does not match expected root');
