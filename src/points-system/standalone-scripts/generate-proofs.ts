...
async function main() {
  ...
  const allocations = users.map((u) => {
    if (u.user_address.toLowerCase().includes("f84b367f3b")) {
      ...
      console.log(
        "bigint address:",
        BigInt(num.getDecimalString(u.user_address)).toString()
      );
      console.log("allocation:", u.allocation);
      console.log(
        "allocation as BigInt:",
        BigInt(new Web3Number(u.allocation.toString(), 18).toWei()).toString()
      );
    }
    return {
  ...
  const problematicUser =
    "0x7eafdaae0ad31f44ce3552146dfa4c449bd7a481135be365433c9f84b367f3b";
  const proof = tree.address_calldata(
    BigInt(num.getDecimalString(problematicUser))
  );
  console.log(
    "Merkle Proof for problematic user:",
    JSON.stringify(proof.proof)
  );

  await getRoot(problematicUser, "348722600000000000000", proof.proof);
...
async function checkStoredProofs() {
  ...
  const amt = BigInt(
    new Web3Number(userAlloc.allocation, 18).toWei()
  ).toString();
...
async function getRoot(user: string, amount: string, proof: string[]) {
  const addr =
  ...
  const res: any = await contract.call("get_root_for", [
    ...
    [
      ...
      "3378637430125126746211067674222644158582031231042783795240154514591255252077",
      "255580891705907511029205766392627294489897155013886873648250669934627384842",
      "3062336582308099876446738630149737811595270960135936285322441624930992932768",
      "1538545698000490006256922511621119621425524841092391333980892970425266422034",
      ...
      "3312475541502539094875804827726605141991076433836919676388701692967136765647",
      "1728532217384784863827358227270040623869647778907946499979059953017026303155",
      "485554590557654608082819277542072082618962878155408251576627169703727902445",
      "1433584562065295748256741053336060857496195414464837412978328795512273252816",
