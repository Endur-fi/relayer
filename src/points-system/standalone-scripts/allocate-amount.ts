...
async function main() {
  ...
  const totalPoints = users.reduce(
    (sum, u) => sum + BigInt(u.total_points),
    0n
  );
  ...
  for (let i = 0; i < users.length; i += BATCH_SIZE) {
    ...
    const batch: any[] = [];
    console.log(
      `Processing batch ${Math.ceil(i / BATCH_SIZE) + 1} of ${Math.ceil(users.length / BATCH_SIZE)}`
    );
    for (const user of batchUsers) {
      const userPoints = BigInt(user.total_points);
      const allocation =
        Number((LUMPSUM_POOL_XSTRK * 10000n * userPoints) / totalPoints - 1n) /
        10000; // Subtract 1wei to avoid rounding issues
  ...
  if (
    netAllocation > LUMPSUM_POOL_XSTRK ||
    netAllocation < LUMPSUM_POOL_XSTRK - BigInt(1e18)
  ) {
    throw new Error(
      `Net allocation mismatch: expected near ${LUMPSUM_POOL_XSTRK}, got ${netAllocation}`
    );
  }
...
function compare() {
  ...
  const origAllocations = JSON.parse(
    fs.readFileSync("allocationsOrig.json", "utf8")
  );

  const missingUsersInLocal = origAllocations.filter(
    (a: any) => !allocations.some((b: any) => b.user_address === a.user_address)
  );
