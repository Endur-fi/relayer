...
async function fetchHoldingsWithRetry(
    ...
    try {
      ...
      if (!blockInfoCache[date.toISOString().split('T')[0]]) {
        throw new Error(`Block info for user ${userAddr} not found in blockInfoCache`);
      }
    ...
    for (const holdings of allHoldings) {
      if (holdings && holdings.opusAmount && holdings.opusAmount !== '0') {
        ...
        if (existing && existing.opusAmount === holdings.opusAmount) {
          console.log(`No update needed for user: ${holdings.user_address} with opusAmount: ${holdings.opusAmount}`);
          continue;
        }
        ...
        records++;
        console.log(`Updated user: ${holdings.user_address} with opusAmount: ${holdings.opusAmount}`, records);
      }
    }
  }, { timeout: 300000 }); // 10 seconds timeout for the transaction
  return records;
}
...
async function handleOpusPoints() {
  ...
  setInterval(() => {
    console.log(`Processed ${count} tasks so far., records updated: ${records}`);
  ...
  for (const [user, date] of tasks) {
    async function processTask(_user: string, _date: Date) {
      try {
        ...
        count++;
        console.error(`Failed to fetch holdings for user: ${_user} on date: ${_date.toISOString()}:`, error);
      }
    }
  ...
  const holdings = results.filter(h => h && h.opusAmount && h.opusAmount !== '0');
  data.push(...holdings);
...
async function storeOpusHoldings() {
  ...
  for (let i=0; i<data.length; i+=50) {
    ...
    console.log(`Saved batch ${Math.ceil(i / 50) + 1} of ${Math.ceil(data.length / 50)} with ${records} records updated.`);
  }
}
